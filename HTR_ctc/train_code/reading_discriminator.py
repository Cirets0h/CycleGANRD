...
class ReadingDiscriminator():
    ...
    def __init__(self, optimizer, net, loss, lr=1e-4, load_model = None, rd_low_loss_learn = False):
    ...
    def train_on_Dataloader(self, epoch, train_loader, test_set, test_loader, scheduler = None, wandb = None, forceLower = False):

        self.optimizer.zero_grad()
        iter_rep = 50
        log_dict = {}
        cer_path_hist, cer_beam_hist, wer_path_hist, wer_beam_hist, cer_beam_clean_hist, wer_beam_clean_hist  = self.testOnHistorical(forceLower)
...
#        np_img = img.detach().squeeze(0).permute(1,2,0).cpu().numpy()
 #       cv2.imwrite('/home/manuel/CycleGANRD/PyTorch-CycleGAN/output/test2.png', np_img*255)
    ...
    def testOnHistorical(self, forceLower=False):
        ...
        file_names = sorted(glob.glob('/home/manuel/CycleGANRD/HTR_ctc/data/historical' + '/*.*'))
        cer_path, wer_path = [], []
        cer_beam, wer_beam = [], []
